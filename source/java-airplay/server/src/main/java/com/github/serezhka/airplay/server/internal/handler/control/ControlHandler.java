...
public class ControlHandler extends ChannelInboundHandlerAdapter {
    ...
    /** 解析 AirPlay SET_PARAMETER 中的 volume 参数，格式: "volume: -30.000000" */
    private static final Pattern VOLUME_PATTERN = Pattern.compile("volume:\\s*(-?[\\d.]+)");
    ...
    private void handleRtspGetParameter(ChannelHandlerContext ctx, FullHttpRequest request) {
        // 返回当前音量（0.0 = 最大），iPhone 会读取此值显示音量条
    ...
    private void handleRtspSetParameter(ChannelHandlerContext ctx, FullHttpRequest request) {
        // 解析 volume 参数（iPhone 侧音量调节时发送）
        // 请求体格式: "volume: -30.000000\r\n"
        // 范围: -144.0（静音）到 0.0（最大）
        String body = request.content().toString(StandardCharsets.US_ASCII);
        log.debug("SET_PARAMETER body: {}", body);

        if (body.contains("volume:")) {
            try {
                var matcher = VOLUME_PATTERN.matcher(body);
                if (matcher.find()) {
                    float volume = Float.parseFloat(matcher.group(1));
                    log.info("AirPlay volume change: {} dB", volume);
                    airPlayConsumer.onVolumeChange(volume);
                }
            } catch (Exception e) {
                log.warn("Failed to parse volume from SET_PARAMETER: {}", e.getMessage());
            }
        }
