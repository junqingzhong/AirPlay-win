...
public class PropertyListUtil {
    ...
    public static byte[] prepareInfoResponse(AirPlayConfig airPlayConfig) throws Exception {
        ...
        // Landscape handling: swap width/height so width >= height, and advertise rotation
        int width = airPlayConfig.getWidth();
        int height = airPlayConfig.getHeight();
        boolean rotate = false;
        if (airPlayConfig.isLandscape()) {
            if (height > width) {
                int t = width;
                width = height;
                height = t;
